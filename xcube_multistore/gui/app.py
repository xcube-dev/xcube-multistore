...
def render_schema(name, schema, prefix=""):
    ...
    if schema.type == "boolean":
        w = pn.widgets.Checkbox(
            name=name,
            value=schema.default if schema.default != UNDEFINED else False
        )
        widgets[full_prefix] = w
        return w, widgets
    ...
    if schema.type == "object":
        ...
        for prop_name, prop_schema in schema.properties.items():
            ui, inner_widgets = render_schema(prop_name, prop_schema, prefix=full_prefix)
            section.append(ui)
...
def on_add_store(event):
    ...
    if any(s["identifier"] == identifier for s in store_entries):
        pn.state.notifications.error(f"A store with identifier '{identifier}' already exists.",
                                         duration=10000)
        modal.hide()
        return
...
gm_bbox = pn.widgets.TextInput(name="BBox (west, south, east, north)",
                               value="2,2,2,2")
...
gm_tile_size = pn.widgets.TextInput(name="Tile Size", value="[1024, 1024]",
                                    )
...
def build_open_params_ui(container, store_id, data_id):
    ...
    try:
        ...
        pn.state.notifications.error(f"Open params error: {e}",
                                         duration=10000)
        return {}
...
ds_data_id = pn.widgets.AutocompleteInput(name="Data ID", options=[],
                                          min_characters=0,
...
def on_ds_store_change(event):
    ...
    if store_id not in DATA_ID_CACHE:
        ...
        try:
            ...
            raw = MultiSourceDataStore.list_data_ids({store["store_id"]: store["store_params"]})
            options = []
...
def on_add_dataset(event):
    entry = {
        ...
        "open_params": {k.split(".")[-1]: w.value for k, w in ds_open_params_widgets.items()}
    }
